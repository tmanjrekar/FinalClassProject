...
write_tsv(sankey_data_allTaxa, "NEON_MAGs_2_pavian.txt")
```
...
# Write file to input to Pavian Sankey
write_tsv(sankey_data_allTaxa, "NEON_MAGs_co_2_pavian.txt")
```
...
Guanica State Forest has a diverse range of phylum with Actinobacteriota being the most prevalent phylum at this site. 
...
```{r}
library(usethis)
use_git_config(user.name = "Tanvi Manjrekar", user.email = "tmanjrekar@umass.edu")
```
